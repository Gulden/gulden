...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, runs_ci):
    ...
    # Warn if Gulden-daemon is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "Gulden-daemon"]) is not None:
            print("%sWARNING!%s There is already a Gulden-daemon process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
