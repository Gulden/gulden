...
class GuldenTestFramework(metaclass=GuldenTestMetaClass):
    ...
    def main(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave Gulden-daemons and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop Gulden-daemons after the test execution")
        ...
        self.config = config
        self.options.guldendaemon = os.getenv("GULDENDAEMON", default=config["environment"]["BUILDDIR"] + '/src/Gulden-daemon' + config["environment"]["EXEEXT"])
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: Gulden-daemons were not stopped and may still be running")
    ...
    def add_nodes(self, num_nodes, extra_args=None, *, rpchost=None, binary=None):
        ...
        if binary is None:
            binary = [self.options.Gulden_daemon] * num_nodes
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a Gulden-daemon test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self, wait=0):
        """Stop multiple Gulden-daemon test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as Gulden-daemon's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if create_cache:
            ...
            # Create cache directories, run Gulden-daemons:
            for i in range(MAX_NODES):
                datadir = initialize_datadir(self.options.cachedir, i)
                args = [self.options.Gulden_daemon, "-datadir=" + datadir, '-disablewallet']
                if i > 0:
                ...
                self.nodes.append(TestNode(
                    ...
                    rpchost=None,
                    timewait=self.rpc_timeout,
                    Gulden_daemon=self.options.guldendaemon,
                    Gulden_cli=self.options.guldencli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                ))
    ...
    def skip_if_no_daemon_zmq(self):
        """Skip the running test if Gulden-daemon has not been compiled with zmq support."""
