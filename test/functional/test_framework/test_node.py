...
"""Class for Gulden-daemon node under test"""
...
class TestNode():
    """A class for representing a Gulden-daemon node under test.
    ...
    def __init__(self, i, datadir, *, rpchost, timewait, Gulden_daemon, Gulden_cli, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False):
        """
        Kwargs:
    ...
    def __del__(self):
        # Ensure that we don't leave any Gulden-daemon processes lying around after
        # the test ends
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, **kwargs):
        ...
        # Add a new stdout and stderr file each time Gulden-daemon is started
        if stderr is None:
        ...
        # unclean shutdown), it will get overwritten anyway by Gulden-daemon, and
        ...
        self.running = True
        self.log.debug("Gulden-daemon started, waiting for RPC to come up")

        if self.start_perf:
            self._start_perf()

    def wait_for_rpc_connection(self):
        """Sets up an RPC connection to the Gulden-daemon process. Returns False if unable to connect."""
        # Poll at a rate of four times per second
        poll_per_s = 4
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                raise FailedToStartError(self._node_msg(
                    'Gulden-daemon exited with status {} during initialization'.format(self.process.returncode)))
            try:
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcassword. Gulden-daemon still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error("Unable to connect to Gulden-daemon")
    ...
    def _start_perf(self, profile_name=None):
        ...
        if not test_success('readelf -S {} | grep .debug_str'.format(shlex.quote(self.binary))):
            self.log.warning(
                "perf output won't be very useful without debug symbols compiled into Gulden-daemon")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to Gulden-daemon
        expected_msg: regex that stderr should match when Gulden-daemon fails

        Will throw if Gulden-daemon starts without an error.
        Will throw if an expected_msg is provided and it does not match Gulden-daemon's stdout."""
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
            ...
            try:
                ...
                self.log.debug('Gulden-daemon failed to start: %s', e)
                self.running = False
                self.process = None
                ...
                if expected_msg is None:
                    assert_msg = "Gulden-daemon should have exited with an error"
                else:
                    assert_msg = "Gulden-daemon should have exited with expected error " + expected_msg
